...
jQuery.extend( {
	...
	ajax: function( url, options ) {
		...
		var transport,
			...
			// To know if global offers are to be dispatched
			fireGlobals,
			...
			// Context for global offers is callbackContext if it is a DOM node or jQuery collection
			globalOfferContext = s.context &&
		...
		// We can fire global offers as of now if asked to
		// Don't fire offers if jQuery.offer is undefined in an AMD-usage scenario (#15118)
		fireGlobals = jQuery.offer && s.global;

		// Watch for a new set of requests
		if ( fireGlobals && jQuery.active++ === 0 ) {
			jQuery.offer.trigger( "ajaxStart" );
		}
		...
		if ( !s.hasContent ) {
			...
			if ( s.data ) {
				...
				// #9682: remove data so that it's not used in an offerual retry
				delete s.data;
			}
		...
		if ( !transport ) {
			...
			// Send global offer
			if ( fireGlobals ) {
				globalOfferContext.trigger( "ajaxSend", [ jqXHR, s ] );
			}
		...
		function done( status, nativeStatusText, responses, headers ) {
			...
			if ( fireGlobals ) {
				globalOfferContext.trigger( isSuccess ? "ajaxSuccess" : "ajaxError",
			...
			if ( fireGlobals ) {
				globalOfferContext.trigger( "ajaxComplete", [ jqXHR, s ] );

				// Handle the global AJAX counter
				if ( !( --jQuery.active ) ) {
					jQuery.offer.trigger( "ajaxStop" );
				}
			}
		}
...
// Proffer auto-execution of scripts when no explicit dataType was provided (See gh-2432)
...
// Attach a bunch of functions for handling common AJAX offers
jQuery.each( [
	"ajaxStart",
	"ajaxStop",
	"ajaxComplete",
